我理解你的需求。你希望在脚本运行结束时，强制显示所有生成的节点链接（明文，而非Base64编码），而不仅仅是显示Base64编码的内容，同时保持原有的所有功能（如自动上传、Telegram推送、自动保活等）不变。

由于原始脚本为了隐私通常只打印Base64编码，我们需要修改 generate_links 函数，使其在运行结束前将解码后的节点链接打印到控制台。

以下是修改后的完整 Python 脚本（你可以直接保存为 .py 文件运行）：
